...
<script>
  export default {
    ...
    methods: {
      ...
      // 显示所有商品
      showAll(){
        this.$http.get("http://localhost:8000/getAllProduct",{
          params:{
            pid:this.pid
          }
        }).then((res)=>{
      ...
      updateProduct(index,row) {
        ...
        if(row.butxt=="编辑"){
          ...
          let newmsg=row.msg;
          this.$http.post("http://localhost:8000/updateProduct",{
            id:row.id,
            newbrand:newbrand,
            newname:newname,
            newprice:newprice,
      ...
      delProduct(myid){
          // console.log(myid.id);
          this.$http.get("http://localhost:8000/delById",{
            params:{
              id:myid.id
            }
          }).then((res)=>{
      ...
      addProduct() {
        if (this.myclass == "hidden") {
          ...
          //同时增加商品
          // console.log(this);
          this.$http.post("http://localhost:8000/uploadFile",{
            brand:this.brand,
            name:this.name,
            price:this.price,
